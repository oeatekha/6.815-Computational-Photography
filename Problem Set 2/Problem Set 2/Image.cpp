...
float Image::smartAccessor(int x, int y, int z, bool clamp) const {
  // --------- HANDOUT  PS02 ------------------------------
    ...
    int y_new = y;
    int z_new = z;
    bool oob; //out of bounds DIDNT USE
    //cout << endl << "values" << endl;
    //cout << endl << "values" << endl;

    //Bound X and Y and Z
    if (x < 0) { x_new = 0; oob = 1; }
    else if (x > width() - 1) { x_new = width() - 1; oob = 1; } //remove one cause 0 indexing
    if (y < 0) { y_new = 0; oob = 1;}
    else if (y > height() - 1) { y_new = height() - 1; oob = 1; }
    if (z < 0) { z_new = 0; oob = 1; }
    else if (z > channels() - 1) { z_new = channels() -1; oob = 1; }
    ...
    // Okay what you did was create new x y z varibles to be edited
    // by doing it we can chack for out of range
    // if none than the values are not altered and we have the normal index or 
    // x y z values if clamp = 1 we return it or the og values.
    // if x y z values and clamp = 0 we return 0...
}
// ---------------- END of PS02 -------------------------------------
