    ...
    for (int j = 0; j < height; j++) {
        for (int i = 0; i < width; i++) {
            ...
            for (int k = 0; k < channels; k++) {
                output(i, j, k) = im(x, y, k); //am i thinking???
            }
        }
    }
...
float interpolateLin(const Image &im, float x, float y, int z, bool clamp) {
  ...
  //                   closest to * should influence it more. is dx/dy the scale
    //Get the locations
    // 1  = (x1, y1)
    // 2  = (x2, y1)
    // 3  = (x2, y2)
    // 4  = (x1. y2)
    // we can interpolate as two seperate interpolations to ease computation time
    int x1 = floor(x);  int x2 = x1 + 1; int y1 = floor(y); int y2 = y1 + 1;  
    float alpha_x = x - x1;
    float alpha_y = y - y1;
    float xy_interp1 = (1 - alpha_x) * im.smartAccessor(x1, y1, z, clamp) + alpha_x* im.smartAccessor(x2, y1, z, clamp); //(1-alpha)*f(x) + alpha*f(x+1)
    float xy_interp2 = (1 - alpha_x) * im.smartAccessor(x1, y2, z, clamp) + alpha_x * im.smartAccessor(x2, y2, z, clamp);
    float output = (1 - alpha_y) * xy_interp1 + alpha_y * xy_interp2;
...
Image scaleLin(const Image &im, float factor) {
  ...
  // bilinear interpolation
    ...
    for (int j = 0; j < height; j++) {
        for (int i = 0; i < width; i++) {
            x = scale * i; //no floor bc we are interpolating
            y = scale * j;
            for (int k = 0; k < channels; k++) {
                //clamp == true;
                output(i, j, k) = interpolateLin(im, x, y, k, true); //am i thinking??? why no clamp.......? // if there as a floor it would do NN interpolation
            }
