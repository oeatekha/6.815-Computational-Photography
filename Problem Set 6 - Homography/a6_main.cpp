...
void HomographySheet() {
    Matrix H(3, 3);
    CorrespondencePair correspondances[4] = { CorrespondencePair(263, 68, 1, 0, 0, 1),
                                   CorrespondencePair(1032, 62, 1, 1040, 0, 1),
                                   CorrespondencePair(149, 1357, 1, 0, 1360, 1),
                                   CorrespondencePair(1032, 1357, 1, 1040, 1360, 1) };
    H = computeHomography(correspondances);


    cout << "Given manual homography for paper example:" << endl;
    cout << H << endl;

    // Load inputs
    Image paper("Input/notebooktxt.png");
    Image corrected(1040, 1360, 3);

    // apply homography
    applyHomography(paper, H, corrected, true);
    corrected.write("Output/paper_homography.png");
}
